    ...
    public class SamplePlayerAnimationController : MonoBehaviour
    {
        ...
        private void FaceMoveDirection()
        {
            ...
            _strafeAngle = characterForward != directionForward ? 
                Vector3.SignedAngle(characterForward, directionForward, Vector3.up) : 0f;
            ...
            if (_isStrafing)
            {
                if (_moveDirection.magnitude > 0.01)
                {
                    if (_cameraForward != Vector3.zero)
                    {
                        ...
                        // what direction to shuffle, and then lock the value so it doesn't return to zero once we lose input
                        // (so the blend tree works to the end of the anim clip)
